...
static int my_ret_handler(struct kretprobe_instance *ri, struct pt_regs *regs)
{
	...
	printk(LOG_LEVEL "return value: %lu\n PID: %d\n  PPID: %d\n", 
		regs_return_value(regs),current->pid, current->parent->pid);
...
static int my_probe_init(void)
{
	...
	my_kretprobe.kp.symbol_name = "_do_fork";	
	ret = register_kretprobe(&my_kretprobe);
	if ( ret < 0) {
		printk(LOG_LEVEL "register_kretprobe failed, returned %d\n", ret);
		return -1;
	}
	printk(LOG_LEVEL "Planted return probe at %s: %p\n",
...
static void my_probe_exit(void)
{
	...
	printk(LOG_LEVEL "kretprobe at %p unregistered\n", my_kretprobe.kp.addr);
}
